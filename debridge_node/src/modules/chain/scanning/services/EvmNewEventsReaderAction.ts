...
@Injectable()
export class EvmNewEventsReaderAction {
  readonly #logger = new Logger(EvmNewEventsReaderAction.name);
  readonly #locker = new Map();
  ...
  readonly #solanaReaderService: SolanaReaderService;
  ...
  constructor(
    ...
    solanaReaderService: SolanaReaderService,
    ...
    this.#solanaReaderService = solanaReaderService;
  ...
  async process(chainId: number, from?: number, to?: number): Promise<void> {
    const logger = new Logger(`${EvmNewEventsReaderAction.name} chainId ${chainId}`);
    ...
    for (fromBlock; fromBlock < toBlock; fromBlock += chainConfig.maxBlockRange) {
      ...
      const sentEvents = await this.#getSentEvents(gateContract, fromBlock, lastBlockOfPage);
      const monitoringSendEvents = await this.#getMonitoringSendEvents(gateContract, fromBlock, lastBlockOfPage);
      ...
      const monitoringEvents = monitoringSendEvents.map(rawEvent => this.#transformService.generateMonitoringSendEventFromEvmEvent(rawEvent));
      const status = await this.#chainProcessingService.process(submissions, monitoringEvents, chainId, lastBlockOfPage, web3);
   ...
   */
  async #getConfirmedBlockNumber(web3: Web3Custom, blockConfirmation: number): Promise<number> {
  ...
  async #getSentEvents(registerInstance: Contract, fromBlock: number, toBlock: number) {
  ...
  async #getMonitoringSendEvents(registerInstance: Contract, fromBlock: number, toBlock: number) {
    if (fromBlock >= toBlock) return;

    /* get events */
    return await registerInstance.getPastEvents('MonitoringSendEvent', { fromBlock, toBlock });
  }
}

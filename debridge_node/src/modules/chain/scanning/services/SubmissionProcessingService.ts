  ...
  async processNewTransfers(logger: Logger, submissions: SubmissionEntity[], chainDetail: ChainConfig): Promise<ProcessNewTransferResult> {
    ...
    for (const submission of submissions) {
      ...
      if (nonceValidationStatus !== NonceValidationEnum.SUCCESS) {
        ...
        const message = `Incorrect nonce (${nonceValidationStatus}) for nonce: ${nonce}; max nonce in db: ${chainMaxNonce}; submissionId: ${submissionId}; blockToOverwrite: ${blockOrTransaction}; submissionWithMaxNonceDb.blockNumber: ${submissionWithMaxNonceDb?.blockNumber}`;

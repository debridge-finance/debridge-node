...
export class ValidationBalanceService {
  ...
  async calculate(
    ...
    try {
      ...
      const balanceSender = await ValidationBalanceService.getBalance(manager, chainFrom, debridgeId);
      const balanceReceiver = await ValidationBalanceService.getBalance(manager, chainTo, debridgeId);
      ...
      if (newBalances.status === SubmisionBalanceStatusEnum.CHECKED) {
        await ValidationBalanceService.setBalance(manager, chainFrom, debridgeId, blockTimestamp, newBalances.sender);
        await ValidationBalanceService.setBalance(manager, chainTo, debridgeId, blockTimestamp, newBalances.reciever);
  ...
  private static async getBalance(entityManager: EntityManager, chainId: number, debridgeId: string): Promise<TokenBalanceHistory> {
  ...
  private static async setBalance(entityManager: EntityManager, chainId: number, debridgeId: string, blockTimestamp: number, amount: BigNumber) {
